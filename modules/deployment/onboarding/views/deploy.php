<style>
    ...
    <div id="finish-panel" style="display:none">
        <?= wizard_step_dots(wizard_step_classes(5, 5)) ?>
        <a href="deployment" class="btn-primary">Go to Dashboard &#10148;</a>
...
(function () {
    ...
    es.addEventListener('done', function (e) {
        ...
        if (result.status === 'missing_zip') {
            ...
            finish.querySelector('.btn-primary').href = 'deployment-onboarding/deployment';
            finish.querySelector('.btn-primary').textContent = 'Upload zip again';
            finish.style.display = '';
            return;
        }

        if (result.status === 'staged' || result.status === 'success') {
            msg.textContent = result.status === 'staged'
                ? 'Release staged. Open the deployment page to promote it.'
                : '✓ Deployment complete!';
        } else {
